...
public class RaslDesign extends Thread {

	public RaslDesign(RaslRuntime rt, String s2) {
		this.blastDB = rt.getBlastDB();
		this.maxNbOligo = rt.getMaxNbOligo();
		this.lengthMin = rt.getLengthMin();
		this.lengthMax = rt.getLengthMax();
		this.distance = rt.getDistance();
		this.betweenOligo = rt.getBetweenOligo();
		this.tmMin = rt.getTmMin();
		this.tmMax = rt.getTmMax();
		this.temperature = rt.getTemperature();
		this.temperatureK = rt.getTemperatureK();
		this.crossHyb = rt.getCrossHyb();
		this.minGC = rt.getMinGC();
		this.maxGC = rt.getMaxGC();
		this.listProhibited = rt.getListProhibited();
		this.concDna = rt.getConcDna();
		RaslDesign _tmp = this;
		this.dataG = rt.getDataG();
		RaslDesign _tmp1 = this;
		this.dataH = rt.getDataH();
		this.currentDir = s2;
	...
	public RaslDesign(SeqDispenser seqdispenser, String s, int i, int j, int k,
			int l, int i1, int j1, int k1, double d, int l1, double d1,
	...
	public void run() {
		try {
			while (!SequenceQueueService.INSTANCE.isSequenceQueueEmpty()){
				Sequence sequence1 = SequenceQueueService.INSTANCE.getSequence();
				...
				if (originalLength >= distance) {
					sequence = sequence1.sequence(originalLength - distance,
							originalLength);
				...
				try {
					...
					logFile.println("For sequence " + seqName
							...
							+ exception1);
					logFile.flush();
					System.err.println("The process crashed due to an error ("
							+ exception1 + ").");
				}
				logFile.flush();
				skipped.flush();
	...
	private boolean analizeSeq(String s) throws IOException {
		...
		expected = OligoArray2.K
				* (OligoArray2.blastDbLength - OligoArray2.nbSeqBlastDb
						* (double) hsp) * (double) (seqLength - hsp)
				* Math.exp(-13D * OligoArray2.lambda) * 1.1000000000000001D;
			...
			if (containsProhibited(oligoSequence, "N")) {
				...
				if (0.0D <= dGStruct) {
					...
					if (l == 1) {
						...
						hashtable2.put(new Integer(begin),
								oligoSequence.sequence());
						hashtable.put(new Integer(begin), thermoData);
						hashtable3.put(new Integer(begin),
								(new Double(dGStruct)).toString());
						Vector vector1;
			...
			for (int k = 0; (k < ainteger.length) & (j < maxNbOligo); k++) {
				...
				for (int i1 = 0; (i1 < vector.size()) & (j < maxNbOligo); i1++) {
					...
					if (isBeginValid(k1, usedPos)) {
						formatOligo(k1,
								(double[][]) hashtable.get(new Integer(k1)),
	...
	private void formatOligo(int i, double ad[][], Hashtable hashtable,
