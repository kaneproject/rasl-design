...
public class Design extends Thread
{
    ...
    public Design(SeqDispenser seqdispenser, String s, int i, int j, int k, int l, int i1, 
            ...
            String s2)
    {
        ...
        tmMax = k1;
        temperature = d;
        temperatureK = d + 273.14999999999998D;
        crossHyb = l1;
        minGC = d1;
        maxGC = d2;
        listProhibited = s1;
        ...
        try
        {
            logFile = new PrintWriter(new FileOutputStream(new File(s2, "OligoArray.log")));
            skipped = new PrintWriter(new FileOutputStream(new File(s2, "rejected.fas")));
            oligoDb = new PrintWriter(new FileOutputStream(new File(s2, "oligos.txt")));
        }
    ...
    public void run()
    {
        try
        {
            for(Sequence sequence1 = disp.get(); sequence1 != null; sequence1 = disp.get())
            {
                ...
                {
                    sequence = sequence1.sequence(originalLength - distance, originalLength);
                ...
                {
                    logFile.println("For sequence " + seqName + ", the following exception was caugth: " + outofmemoryerror);
                    logFile.flush();
                    System.err.println("The Blast crashed because there is not enought memory allowed to the application (" + outofmemoryerror + ").\nYou may want to allow more memory to java by using the java option -Xmx256m (will allow 256 M RAM)");
                }
                catch(Exception exception1)
                {
                    logFile.println("For sequence " + seqName + ", the following exception was caugth: " + exception1);
                    logFile.flush();
                    System.err.println("The process crashed due to an error (" + exception1 + ").");
                }
                logFile.flush();
                skipped.flush();
                oligoDb.flush();
    ...
    private boolean analizeSeq(String s)
        throws IOException
    {
        ...
        boolean flag = false;
        hsp = OligoArray2.getHSP(seqLength);
        expected = OligoArray2.K * (OligoArray2.blastDbLength - OligoArray2.nbSeqBlastDb * (double)hsp) * (double)(seqLength - hsp) * Math.exp(-13D * OligoArray2.lambda) * 1.1000000000000001D;
        logFile.print("Running Blast (-e " + expected + ")... ");
        logFile.flush();
        Blast blast = new Blast(blastDB, "blastn", "-S 1 -F F -W 7 -v 5 -b 10000 -e " + expected);
        ...
        while((begin > 0) & (seqLength - begin < distance) & (j < maxNbOligo)) 
        {
            ...
            {
                logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to prohibited sequences");
                end = end - 1;
                begin = begin - 1;
            } else
            if(oligoSequence.containsDiRepeat(10) || oligoSequence.containsTriRepeat(10))
            {
                logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to repeated sequences");
                end = end - 1;
                begin = begin - 1;
            } else
            if(maxGC < d)
            {
                logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to high percent of GC: " + d);
                end = end - 1;
                begin = begin - 1;
            } else
            if(minGC > d)
            {
                logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to low percent of GC: " + d);
                end = end - 1;
                begin = begin - 1;
            } else
            if(oligoSequence.tmNN(concDna) < (double)tmMin)
            {
                if(i >= lengthMax)
                {
                    logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to low Tm: " + oligoSequence.tmNN(concDna));
                    end--;
                    begin--;
                } else
                {
            ...
            if(oligoSequence.tmNN(concDna) >= (double)tmMax)
            {
                if(i > lengthMin)
                {
                    end--;
                ...
                {
                    logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to high Tm: " + oligoSequence.tmNN(concDna));
                    end--;
                    begin--;
                }
            } else
            ...
            } else
            {
                ...
                if(0.0D <= dGStruct)
                {
                    ...
                    if(l == 1)
                    {
                        formatOligo(begin, thermoData, oligoSequence.sequence(), (new Double(dGStruct)).toString());
                        logFile.println(seqName + "\t" + (begin + 1) + "\tOligo selected");
                    ...
                    if(!flag)
                    {
                        hashtable2.put(new Integer(begin), oligoSequence.sequence());
                        hashtable.put(new Integer(begin), thermoData);
                        hashtable3.put(new Integer(begin), (new Double(dGStruct)).toString());
                        Vector vector1;
                        if(hashtable1.containsKey(new Integer(l)))
                            vector1 = (Vector)hashtable1.get(new Integer(l));
                        else
                            vector1 = new Vector();
                        vector1.add(new Integer(begin));
                        hashtable1.put(new Integer(l), vector1);
                        logFile.println(seqName + "\t" + (begin + 1) + "\tNon specific oligo ignored at this time");
                        end = end - 1;
                        begin = begin - 1;
                    } else
                    {
                        logFile.println(seqName + "\t" + (begin + 1) + "\tNon specific oligo rejected");
                        end = end - 1;
                        begin = begin - 1;
                    }
                } else
                {
                    logFile.println(seqName + "\t" + (begin + 1) + "\trejected due to a secondary structure (deltaG = " + dGStruct + " @ " + temperature + " degrees)");
                    end = end - 1;
                    begin = begin - 1;
                }
                logFile.flush();
        ...
        if((!flag) | (j < maxNbOligo))
        {
            ...
            {
                Vector vector = (Vector)hashtable1.get(ainteger[k]);
                for(int i1 = 0; (i1 < vector.size()) & (j < maxNbOligo); i1++)
                {
                    int k1 = ((Integer)vector.elementAt(i1)).intValue();
                    if(isBeginValid(k1, usedPos))
                    {
                        formatOligo(k1, (double[][])hashtable.get(new Integer(k1)), hashtable2, hashtable3);
                        logFile.println(seqName + "\t" + (k1 + 1) + "\tNon specific oligos selected");
                        flag = true;
                        j++;
    ...
    private boolean isBeginValid(int i, Vector vector)
    {
        boolean flag = true;
        for(int j = 0; j < vector.size(); j++)
        {
            int k = ((Integer)vector.elementAt(j)).intValue() - betweenOligo;
    ...
    private static boolean containsProhibited(Sequence sequence1, String s)
    {
        ...
        for(StringTokenizer stringtokenizer = new StringTokenizer(s, ";"); stringtokenizer.hasMoreTokens() & (!flag);)
    ...
    private static String maskSequence(String s, String s1)
    {
        for(StringTokenizer stringtokenizer = new StringTokenizer(s1, ";"); stringtokenizer.hasMoreTokens();)
        {
            ...
            int k;
            for(; s.indexOf(s2) != -1; s = s.substring(0, s.indexOf(s2)) + generateN(k) + s.substring(s.indexOf(s2) + k))
            {
                k = i;
                if(flag)
                {
                    for(int l = s.indexOf(s2); l + k < s.length() && c == s.charAt(l + k); k++);
                }
            }
    ...
    private void formatOligo(int i, double ad[][], Hashtable hashtable, Hashtable hashtable1)
    {
        String s = "";
        String s1 = "";
        String s2 = "";
        ...
        for(int j = 0; j < ad.length; j++)
        {
            ...
            if(ad[j][3] >= (double)crossHyb)
            {
                ...
                String s11 = "";
                for(int k = i; k < i + ((String)hashtable.get(new Integer(i))).length(); k++)
        ...
        Sequence sequence1 = new Sequence((String)hashtable.get(new Integer(i)));
        ...
        oligoDb.print(seqName + "\t" + (i + deletedLength + 1) + "\t" + sequence1.length() + "\t");
    ...
    private void updateThermoData()
    {
        ...
        for(int k = 0; k < nbAlignment; k++)
        {
            ...
            if(s1.length() > 12)
            {
                ...
                for(; j1 + 2 < i && !(as1[j1].equals(as[j1]) & as1[j1 + 1].equals(as[j1 + 1]) & as1[j1 + 2].equals(as[j1 + 2])); j1++);
                if(j1 + 1 < i)
                    ...
                    if(as1[j1].equals("G") | as1[j1].equals("C"))
                    {
                        f = 0.98F;
                        f1 = 0.1F;
                    }
                for(j1++; j1 < i; j1++)
                    ...
                    if(as1[j1].equals("A") & as[j1].equals("G") | as1[j1].equals("C") & as[j1].equals("T"))
                    {
                    ...
                    } else
                    {
                        String s2 = as1[j1 - 1];
                        String s3 = as[j1 - 1];
                        for(; j1 + 1 < i && (!as1[j1].equals(as[j1]) || !as1[j1 + 1].equals(as[j1 + 1])); j1++)
                        {
                            if(!as1[j1].equals("-"))
                                s2 = s2 + as1[j1];
                            s3 = s3 + as[j1];
                        ...
                        if(j1 + 1 != i)
                        {
                            ...
                            if((s3.length() == 3) & (s2.length() > 3) & (s3.substring(1, 2).equals(s2.substring(1, 2)) | s3.substring(1, 2).equals(s2.substring(s2.length() - 2, s2.length() - 1))))
                            {
                            ...
                            } else
                            {
                                ...
                                f = f + dataG[625][l3] + dataG[convertCode(s3.substring(0, 2))][convertCode(s2.substring(0, 2))] + dataG[convertCode(s3.substring(s3.length() - 2))][convertCode(s2.substring(s2.length() - 2))] + (float)k4 * 0.16F;
                                f1 = f1 + dataH[625][l3] + dataH[convertCode(s3.substring(0, 2))][convertCode(s2.substring(0, 2))] + dataH[convertCode(s3.substring(s3.length() - 2))][convertCode(s2.substring(s2.length() - 2))];
                            }
                        }
                    }
                ...
                float f5 = (1000F * f1) / (f3 + (float)(1.9872000217437744D * Math.log(concDna / 4D))) - 273.15F;
    ...
    public static double dGStructure(String s, double d)
        throws IOException
    {
        if(s.indexOf(">") != -1)
        ...
        String s1 = "hybrid-ss-min -nDNA -t" + d + " -T" + d + " -M0 -N1 -q " + s;
